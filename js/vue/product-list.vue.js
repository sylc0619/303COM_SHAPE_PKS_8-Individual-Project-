var app = new Vue({
  ...
  data: {
    ...
    searchKey: "",
    oriProductList: [],
    categorys: ["All", "Nendoroid", "PVC Figure", "Figure"],
  ...
  computed: {
    computedProductList() {
      ...
      if (
        ...
        result = this.productList.filter(
          (product) =>
            product.ProductName.toUpperCase().indexOf(
              this.searchKey.toUpperCase()
            ) >= 0
        );
        result = result.filter(
          (product) =>
            this.minPrice <= product.Price && this.maxPrice >= product.Price
        );
        if (this.selectedCategory.indexOf("All") < 0) {
          var result = result.filter(
            (product) => this.selectedCategory.indexOf(product.Category) >= 0
          );
        }
        console.log(result);
      } else {
                ...
                if (r == "First_Time_Add_Cart") {
                  ...
                  $.ajax({
                    ...
                    success: function (data, status, xhr) {
                      ...
                      $(".header-top-menu .icon-cart a").attr(
                        "data-quantity",
                        totalQty
                      );
                    },
