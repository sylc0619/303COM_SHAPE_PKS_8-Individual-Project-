...
app.get(['/delivery', '/terms', '/privacy', '/cookie', '/regulatory', '/aboutus', '/location', '/faq', '/contactus', ], function(req, res) {
    res.redirect("/404");
});
...
app.use(function(req, res) {
            ...
            return req.on("data", function(data) {
                ...
                MongoClient.connect(dbUrl, function(err, db) {
                    ...
                    dbo.collection("Cart").find({
                        $and: [usernameCheck, productCheck],
                    }).toArray(function(err, result) {
                        if (result.length > 0) {
    ...
    } else if (/^\/[a-zA-Z0-9-._\/]*.webp$/.test(req.url.toString())) {
        sendFileContent(res, req.url.toString().substring(1), "image/webp");
